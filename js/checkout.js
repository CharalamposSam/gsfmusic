...
decrement.forEach(d => {
  d.addEventListener('click', function () {
    ...
    if (currentValue > 1) {
      ...
      // Decrease amount
      let a = parseInt(this.parentElement.parentElement.querySelector('.amount').textContent)
      a -= +this.parentElement.parentElement.previousElementSibling.getAttribute('data-amount')
      this.parentElement.parentElement.querySelector('.amount').textContent = a + '€'
      ...
      sumA -= +this.parentElement.parentElement.previousElementSibling.getAttribute('data-amount')
...
deleteProduct.forEach(del => {
  del.addEventListener('click', function () {
    ...
    // Update sum amount
    let sumA = parseInt(sumAmount.textContent)
    sumA -= +this.parentElement.parentElement.previousElementSibling.getAttribute('data-amount') * +this.parentElement.parentElement.querySelector('input').value
    sumAmount.textContent = sumA + '€'
